...
from chat_core.core.data.utils import jsonify_data
from chat_core.utils.connector import DialogLogger
from chat_core.utils.server import get_server_params
...
def encode(data: Any) -> bytes:
    ...
    Examples:
        >>> from chat_core.utils.socket import encode
        >>> encode({'a':1})
...
class SocketServer:
    """Creates socket server that sends the received data to the DeepChat model and returns model response.
    ...
    async def _handle_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        ...
        Validates requests, sends request body to DeepChat model, sends responses to client.
    ...
    def _response(status: str = 'OK', payload: Optional[List[Tuple]] = None) -> bytes:
        ...
        Args:
            ...
            payload: DeepChat model result if no error has occurred, otherwise None.
