...
from chat_core.core.common.file import read_json, find_config
from chat_core.core.common.registry import inverted_registry
...
def _get_variables_from_config(config: Union[str, Path, dict]):
    ...
    variables = {
        'DEEPCHAT_PATH': os.getenv(f'DP_DEEPCHAT_PATH', Path(__file__).parent.parent.parent)
    }
...
def _update_requirements(config: dict) -> dict:
    """
    Generates requirements for DeepChat model and adds them as ``metadata.requirements`` field to the returned dict.
