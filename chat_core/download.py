...
import chat_core
from chat_core.core.commands.utils import expand_path, parse_config
from chat_core.core.data.utils import download, download_decompress, get_all_elems_from_json, file_md5, \
...
def get_configs_downloads(config: Optional[Union[str, Path, dict]] = None) -> Dict[str, Set[Path]]:
    ...
    if config:
        ...
        configs = list(Path(chat_core.__path__[0], 'configs').glob('**/*.json'))
...
def deep_download(config: Union[str, Path, dict]) -> None:
    ...
    for file_id, (url, dest_paths) in enumerate(downloads.items()):
        headers = {
            ...
            'dp-version': chat_core.__version__
        }
